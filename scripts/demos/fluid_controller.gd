...
func _setup_ui() -> void:
	...
	# Solver-calibrated sliders: _configure_solver overwrites several of these on
	# every mode/solver/scenario switch (lava viscosity 0.3, cohesion 800...).
	# _resync_solver_widgets() re-reads the getters and pushes them back into the
	# widgets so the panel always shows what is actually running.
	_solver_pairs = [
		[menu.add_slider("Viscosity", 0.0, 0.5, fluid.get_pbf_viscosity(), fluid.set_pbf_viscosity),
			fluid.get_pbf_viscosity],
		[menu.add_slider("Vorticity", 0.0, 0.1, fluid.get_pbf_vorticity(), fluid.set_pbf_vorticity),
			fluid.get_pbf_vorticity],
		[menu.add_slider("Cohesion", 0.0, 0.01, fluid.get_pbf_cohesion(), fluid.set_pbf_cohesion),
			fluid.get_pbf_cohesion],
		[menu.add_slider("Iterations", 1.0, 6.0, float(fluid.get_pbf_iterations()),
			fluid.set_pbf_iterations), fluid.get_pbf_iterations],
	]
	menu.end_group()

	sph_group = menu.add_group()
	_solver_pairs.append([menu.add_slider("Pressure", 50.0, 600.0, fluid.get_sph_pressure(),
		fluid.set_sph_pressure), fluid.get_sph_pressure])
	_solver_pairs.append([menu.add_slider("Near pressure", 0.0, 80.0, fluid.get_sph_near_pressure(),
		fluid.set_sph_near_pressure), fluid.get_sph_near_pressure])
	# Above ~0.31 the unnormalized XSPH sum overshoots the local mean and
	# oscillates at rest density (lava ships exactly 0.3), so the slider stops
	# at the stable ceiling.
	_solver_pairs.append([menu.add_slider("Viscosity", 0.0, 0.3, fluid.get_sph_viscosity(),
		fluid.set_sph_viscosity), fluid.get_sph_viscosity])
	_solver_pairs.append([menu.add_slider("Cohesion", 0.0, 2000.0, fluid.get_sph_cohesion(),
		fluid.set_sph_cohesion), fluid.get_sph_cohesion])
	_solver_pairs.append([menu.add_slider("Bounce", 0.0, 0.95, fluid.get_sph_bounce(),
		fluid.set_sph_bounce), fluid.get_sph_bounce])
	_solver_pairs.append([menu.add_slider("Sub-steps", 1.0, 6.0, float(fluid.get_sph_substeps()),
		fluid.set_sph_substeps), fluid.get_sph_substeps])
	# Foam widgets never persist: these defaults are recalibrated with the sim
	# (Ihmsen re-fit, 2026-09-23), and a value dragged under an older build must
	# not shadow the new calibration at boot.
	menu.add_slider("Foam amount", 0.0, 300.0, fluid.get_sph_foam_amount(),
		fluid.set_sph_foam_amount, false)
	menu.add_slider("Foam threshold", 0.5, 12.0, fluid.get_sph_foam_threshold(),
		fluid.set_sph_foam_threshold, false)
	menu.add_slider("Foam life", 2.0, 30.0, fluid.get_sph_foam_life(),
		fluid.set_sph_foam_life, false)
	menu.end_group()
	...
	# persist=false: the quality tier owns the particle count (quality.bind pushes
	# it without emitting item_selected, so a hand-persisted index would fight the
	# restored tier at every boot — double re-init, panel showing the wrong tier).
...
func _on_method_selected(idx: int) -> void:
	...
	_update_title()
	_update_param_groups()
	_resync_solver_widgets()


func _on_scenario_selected(idx: int) -> void:
	fluid.set_scenario(idx as FluidSystem.Scenario)
	_update_scenario_ui()
	if fluid.scenario == FluidSystem.Scenario.CASCADE:
		solver_option.item_selected.emit(fluid.method)
	_resync_solver_widgets()
...
## Capture-harness hook: switches the preset through the same runtime path the
## Preset option uses, so captures show a state the user can actually reach.
func set_capture_cascade(value: int) -> void:
	if value >= 0 and value != fluid.scenario:
		_on_scenario_selected(value)
...
## Mode/solver/scenario switches recalibrate solver params in
## _configure_solver; re-read the getters and push them back into the widgets
## so the panel shows what is actually running.
func _resync_solver_widgets() -> void:
	for pair in _solver_pairs:
