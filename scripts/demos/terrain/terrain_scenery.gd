class_name TerrainScenery extends RefCounted
## Everything the terrain demo draws around the solver: the displaced sheet
## and its material, the floor, the sandbox rim, the brush marker, the dust
## puff and the ambience helpers (preset sun/sky/fog, snowfall). Built once
## from the domain size, then only written through.
...
func build(host: Node3D, world_size: float, mesh_n: int) -> void:
	...
	snowfall = _build_snowfall()
	host.add_child(snowfall)
...
func apply_ambience(env: Environment, sun: DirectionalLight3D, preset: TerrainPreset) -> void:
	...
	if sky != null:
		...
		sky.ground_bottom_color = preset.sky_top.darkened(0.55)
		sky.ground_horizon_color = preset.sky_horizon
	env.fog_enabled = preset.fog_density > 0.0
	env.fog_light_color = preset.fog_color
	env.fog_density = preset.fog_density
	env.tonemap_exposure = preset.exposure


func set_snowfall(on: bool) -> void:
...
func _build_dust() -> GPUParticles3D:
	...
	ramp.add_point(0.15, Color(0.85, 0.72, 0.52, 0.20))
...
# Ambience snowfall: a slow drift of small white flakes across the whole
# domain, high above the surface so they land with room to spare.
func _build_snowfall() -> GPUParticles3D:
	var p := GPUParticles3D.new()
	p.amount = 700
	p.lifetime = 7.0
	p.emitting = false
	...
	pm.emission_box_extents = Vector3(_world_size * 1.2, 0.1, _world_size * 1.2)
	pm.direction = Vector3(0, -1, 0)
	pm.spread = 12.0
	pm.initial_velocity_min = 0.35
	pm.initial_velocity_max = 0.7
	pm.gravity = Vector3(0.05, -0.25, 0.03)
	pm.scale_min = 0.35
	pm.scale_max = 0.8
	p.process_material = pm
	var quad := QuadMesh.new()
	...
	var dm := StandardMaterial3D.new()
	dm.albedo_color = Color(1.0, 1.0, 1.0, 0.85)
	# Soft radial falloff — an untextured quad reads as a hard square.
	var soft := GradientTexture2D.new()
	soft.width = 128
	soft.height = 128
