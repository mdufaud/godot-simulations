...
# C# field defaults are NOT what he ships with): a million-particle budget is what
# makes foam read as froth rather than as scattered specks. Only the trapped-air
# threshold is ours: measured here, that metric peaks around 32 and sits at 10 for
# the 99th percentile, so his 15 gates all but a handful of particles out.
...
var foam_bubble_buoyancy := 1.5
var foam_spray_drag := 0.04
var foam_bubble_scale := 0.5
...
func foam_cap() -> int:
	return foam_tex_width * foam_tex_width
...
func init_render() -> void:
	...
	_buffers["foam_count"] = _rd.storage_buffer_create(4, zero_counter)
	...
	# Bindings 10/15 are images and 16 is the foam uniform buffer; the foam pool
	# does not ping-pong, so both parities bind the same foam resources.
	var binding_orders := [
...
func _pack_foam_ubo(frame_dt: float) -> PackedByteArray:
	...
	b.encode_float(0, foam_spawn_rate)
